...
void predict(const sensor_msgs::ImuConstPtr &imu_msg) {
  ...
  Eigen::Vector3d un_acc_1 =
      tmp_Q * (linear_acceleration - tmp_Ba) - estimator_ptr->g;
...
bool getMeasurements(std::vector<sensor_msgs::ImuConstPtr> &imu_msg,
  ...
  if (imu_buf.empty() || feature_buf.empty() ||
      (GNSS_ENABLE && gnss_meas_buf.empty()))
    return false;
  ...
  if (GNSS_ENABLE) {
    ...
    while (!gnss_meas_buf.empty() &&
           front_gnss_ts < front_feature_ts - MAX_GNSS_CAMERA_DELAY) {
  ...
  while (imu_buf.front()->header.stamp.toSec() <
         img_msg->header.stamp.toSec() + estimator_ptr->td) {
    imu_msg.emplace_back(imu_buf.front());
    imu_buf.pop();
  }
  imu_msg.emplace_back(imu_buf.front());
  if (imu_msg.empty()) ROS_WARN("no imu between two image");
  return true;
}
...
void gnss_iono_params_callback(const StampedFloat64ArrayConstPtr &iono_msg) {
  ...
  std::copy(iono_msg->data.begin(), iono_msg->data.end(),
            std::back_inserter(iono_params));
...
void gnss_meas_callback(const GnssMeasMsgConstPtr &meas_msg) {
  ...
  // cerr << "gnss ts is " << std::setprecision(20) <<
  // time2sec(gnss_meas[0]->time) << endl;
...
void feature_callback(const sensor_msgs::PointCloudConstPtr &feature_msg) {
  ...
  if (skip_parameter < 0 && time_diff_valid) {
    const double this_feature_ts =
        feature_msg->header.stamp.toSec() + time_diff_gnss_local;
    if (latest_gnss_time > 0 && tmp_last_feature_time > 0) {
      if (abs(this_feature_ts - latest_gnss_time) >
          abs(tmp_last_feature_time - latest_gnss_time))
        skip_parameter =
            feature_msg_counter % 2;  // skip this frame and afterwards
      else
        skip_parameter =
            1 - (feature_msg_counter % 2);  // skip next frame and afterwards
    }
    // cerr << "feature counter is " << feature_msg_counter << ", skip parameter
    // is " << int(skip_parameter) << endl;
...
void local_trigger_info_callback(
    const gvins::LocalSensorExternalTriggerConstPtr &trigger_msg) {
...
void process() {
  while (true) {
    ...
    con.wait(lk, [&] { return getMeasurements(imu_msg, img_msg, gnss_msg); });
    lk.unlock();
    m_estimator.lock();
    ...
    for (auto &imu_data : imu_msg) {
      ...
      if (t <= img_t) {
        ...
        estimator_ptr->processIMU(dt, Vector3d(dx, dy, dz),
                                  Vector3d(rx, ry, rz));
        ...
        estimator_ptr->processIMU(dt_1, Vector3d(dx, dy, dz),
                                  Vector3d(rx, ry, rz));
    ...
    if (GNSS_ENABLE && !gnss_msg.empty()) estimator_ptr->processGNSS(gnss_msg);
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber sub_feature =
      n.subscribe("/gvins_feature_tracker/feature", 2000, feature_callback);
  ros::Subscriber sub_restart =
      n.subscribe("/gvins_feature_tracker/restart", 2000, restart_callback);
  ...
  if (GNSS_ENABLE) {
    ...
    sub_glo_ephem =
        n.subscribe(GNSS_GLO_EPHEM_TOPIC, 100, gnss_glo_ephem_callback);
    sub_gnss_meas = n.subscribe(GNSS_MEAS_TOPIC, 100, gnss_meas_callback);
    sub_gnss_iono_params =
        n.subscribe(GNSS_IONO_PARAMS_TOPIC, 100, gnss_iono_params_callback);

    if (GNSS_LOCAL_ONLINE_SYNC) {
      sub_gnss_time_pluse_info =
          n.subscribe(GNSS_TP_INFO_TOPIC, 100, gnss_tp_info_callback);
